...
rivet4_env = set_environment_variables(
    os.path.expandvars("$ANALYSIS_PATH/setup/setup_rivet4.sh")
)
...
def get_default_rivet_analyses():
    ...
    default_rivet_env = copy(rivet4_env).update({"RIVET_ANALYSIS_PATH": ""})
...
class Rivet4Build(HTCondorWorkflow, law.LocalWorkflow, BaseTask):
    """
    ...
    rivet_os_version = rivet4_env["RIVET_OS_DISTRO"]
    ...
    def output(self):
        return self.remote_target(f"Rivet_{self.branch_data}.so")
    ...
    def run(self):
        ...
        run_command(_rivet_exec, env=rivet4_env)
